...
[examples gallery](https://github.com/inferogenesis/cpomdp/blob/main/examples/README.md).
...
```python
cue = (CallableGaussianObservation(sensor_model, cue_noise, params)  # B: R(x), alive
       if epistemic_alive
       else GaussianObservation(sensor_model, fixed_noise))          # A: fixed, dead
```
...
![Two agents on the same maze: B reads a state-dependent cue and crosses to the reward, A with a fixed sensor cannot](https://raw.githubusercontent.com/inferogenesis/cpomdp/main/docs/assets/epistemic_dissociation.gif)
...
![A branching tree resolved two ways: CouplingGraph.infer against the hand-flattened joint precision](https://raw.githubusercontent.com/inferogenesis/cpomdp/main/docs/assets/coupling_graph.png)
...
![The E. coli chemotaxis network as a branching factor graph; the hidden CheA kinase hub inferred through its downstream readouts](https://raw.githubusercontent.com/inferogenesis/cpomdp/main/docs/assets/chemotaxis.png)
