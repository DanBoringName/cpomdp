...
# --- scope: tier-1 fixed-matrix only ------------------------------------------


class TestChainBackendScope:
    def test_rejects_state_dependent_sensor(self):
        # R(x) can't be carried by the fixed-matrix observation factor.
        model = LinearGaussianModel(
            ...
            observation=CallableSensor([[C]], lambda x, p: jnp.array([[R]]), None),
        )
        with pytest.raises(ValueError, match="state-dependent"):
            ChainBackend(model)

    def test_rejects_state_dependent_process_noise(self):
        model = LinearGaussianModel(
            dynamics=[[A]],
            sensor_model=[[C]],
            dynamics_noise=[[Q]],
            sensor_noise=[[R]],
            prior=Belief(mean=[0.0], cov=[[1.0]]),
            process_noise=CallableProcessNoise(lambda x, p: jnp.array([[Q]]), None),
        )
        with pytest.raises(ValueError, match="state-dependent"):
            ChainBackend(model)
