...
*3b — Exhaustive enumeration over a finite domain.* Where the quantified set is finite and fully enumerated, execution *decides* the universal, since ¬∃ ≡ ∀¬ and the check is complete. This is live here: policy enumeration over a small action set at feasible H is a finite set, so "the argmin over the enumerated set is prior-ward" (the H\* measurement, §10) is **proved** at each horizon, not corroborated. Note the contrast with 3a: an action *sweep* over a continuous range is a finite grid over an infinite domain and only corroborates. The distinction is whether the quantified domain is finite, not whether the computation is.
...
**The measured result (v0.4.4): H\* = 7.** The exhaustive argmin over `A^H` on the registered `crossover-v1` set `{−2,−1,0,1,2}` is prior-ward (a reach) for H = 1..6 and flips to a two-phase walk `[+1,−2,−2,0,0,0,0]` at H = 7, cue-ward at H = 7, 8, and 9 (1.95M policies at H = 9). This is the decisive measurement, and it is selection-free: each horizon is a complete finite enumeration (Prover 3b), so no policy is chosen with knowledge of the outcome. H\* = 7 is an *upper bound* — `crossover-v1` clips the reach at the grid edge `−2`; on the six-action set containing `−3`, which reaches the goal in one step from the start, the flip is at H\* = 6. Wider sets are unmeasured, and `−3` is not established as optimal: the walk arrives at the cue at `x = +1`, from where the goal is a displacement of `−4`. Both are the result; 7 is the pre-registered number. **Feasibility bound: H_max = 9** (`|A|^H · H = 5⁹·9 = 17.6M` scored steps, measured); any larger H_max is a declared compute-budget increase, not an open question. The full measurement, with conditioning and an independent NumPy kernel, is in `examples/ffg/crossover.py` (gated) and `research/r10_open_loop_crossover.md`.
